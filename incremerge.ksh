...
function show_usage {
echo "usage: incremerge.ksh -r <RMAN login> -h <host> -o <Oracle_sid> -w <recovery window> -t <backup type> -m <Mount point> -s <Linux server>  -c <concurrent S
treams>"
...
#echo $ttype, $mount, $retday, $oraclesid
...
catalog_ksh=$DIR/log/$host/$oraclesid.catalog.ksh
...
#echo file is $DIR/${host}-${oraclesid}-datafile
     ...
     print "$rmanlogin log $catalog_log << EOF" >> $catalog_ksh
     ...
     print "backup as copy current controlfile format '$control_dir/$ORACLE_SID.ctl.$DATE_SUFFIX';"  >> $catalog_ksh

...
function show_usage {
echo "usage: incremerge.bash -h <host> -o <Oracle_sid> -w <recovery window> -t <backup type> -m <Mount point> -s <Linux server>"
...
catalog_bash=$DIR/$host.$oraclesid.catalog.bash
...
#make directory
    ...
    if [[ ! -d "$restore_dir" ]]; then
       echo "Directory $restore_dir does not exist, create it"
       if mkdir -p ${restore_dir}; then
          echo "${restore_dir} is created"
       fi
    fi
...
#     echo ". /home/oracle/.bash_profile" >> $catalog_bash
     ...
     echo "rman target / log $catalog_log << EOF" >> $catalog_bash
